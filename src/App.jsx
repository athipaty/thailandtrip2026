      ...
      <div className={`bg-white border rounded-2xl overflow-hidden transition-all
          ...
          <div className="flex-1 min-w-0">
            ...
            {!isOpen && (
              <div className="flex items-center gap-2 mt-0.5 flex-wrap">
                <span className="text-xs text-gray-500">{day.activities.length} activities</span>
                {day.weather && (
                  <span className="text-[11px] font-semibold text-sky-700">
                    {day.weather.icon} {day.weather.high}°C / {day.weather.low}°C
                  </span>
                )}
              </div>
            )}
        ...
        {isOpen && (
          <div className="border-t border-gray-100 px-4 pb-5">
            ...
            {/* Weather */}
            {day.weather && (
              <div className="flex items-center gap-3 mt-4 px-3 py-2.5 bg-sky-50 border border-sky-100 rounded-xl">
                <span className="text-2xl shrink-0">{day.weather.icon}</span>
                <div className="flex-1 min-w-0">
                  <div className="text-sm font-semibold text-sky-800">{day.weather.desc}</div>
                  <div className="text-xs text-sky-600 mt-0.5">High {day.weather.high}°C · Low {day.weather.low}°C</div>
                </div>
              </div>
            )}
