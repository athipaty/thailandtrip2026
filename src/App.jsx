  ...
  return (
    <div className="flex items-center gap-3 my-2 px-1">
      ...
      <div className="flex items-center gap-1.5 text-xs font-semibold text-gray-500 uppercase tracking-widest">
        ...
        <span className="text-gray-500 font-normal normal-case tracking-normal">{dates}</span>
      </div>
      <div className="flex-1 h-px bg-gray-200" />
    </div>
  )
      ...
      <div className={`bg-white border rounded-2xl overflow-hidden transition-all shadow-sm ${isOpen ? 'border-brand shadow-orange-100 shadow-md' : 'border-gray-200 hover:border-gray-300 hover:shadow-md'}`}>
        ...
        >
          <div className="flex items-center gap-3 flex-1 min-w-0">
            ...
            <div className="min-w-0">
              <div className="flex items-center gap-2 mb-0.5 flex-wrap">
                <span className="text-xs text-gray-500">Day {day.day} · {day.date}</span>
          ...
          <div className="flex items-center gap-3 shrink-0 ml-3">
            <span className="hidden sm:block text-xs text-gray-500 bg-gray-100 border border-gray-200 px-2 py-1 rounded-lg">{day.budget}</span>
            ...
            <div className="pt-4 flex flex-col gap-3">
              {day.activities.map((a, i) => (
                <div key={i} className="grid gap-x-2.5 gap-y-1 items-start" style={{ gridTemplateColumns: '48px 12px 1fr' }}>
                  <span className="text-xs font-semibold text-gray-500 text-right pt-0.5">{a.time}</span>
          ...
          {tripInfo.legs.map((leg, i) => (
            <div key={i} className="flex items-center gap-2 px-3 py-2 rounded-xl bg-white/25 border border-white/30 backdrop-blur-sm">
              ...
              <div>
                ...
                <div className="text-[10px] text-white/75">{leg.hotelNote}</div>
              </div>
            </div>
          ))}
        </div>
      ...
      <main className="max-w-2xl mx-auto px-4 py-5 pb-20">
        ...
        {tab === 'itinerary' && (
          <div className="flex flex-col gap-2.5">
            <div className="bg-white border border-gray-200 rounded-2xl px-4 py-3 text-sm text-gray-600 shadow-sm mb-1">
        ...
        {tab === 'flights' && (
          <div className="flex flex-col gap-3">
            ...
            <p className="text-center text-sm text-gray-600 pt-1">
              Booking ref: <strong className="text-gray-800">{tripInfo.flight.outbound.ref}</strong> · Scoot Airlines
            </p>
          </div>
        )}
